...
Class Master extends DBConnection {
	...
	function save_product(){
		...
		$check = $this->conn->query("SELECT * FROM `products` where `product` = '{$product}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
	...
	function place_order(){
		...
		if($save_order){
			...
			$data = '';
			$cart = $this->conn->query("SELECT c.*,p.product,i.price,p.id as pid from `cart` c inner join `inventory` i on i.id=c.inventory_id inner join products p on p.id = i.product_id where c.client_id ='{$client_id}' ");
